...
def preprocess_image(image_path,count):

    image = cv2.imread(image_path)
    preprocessor = preprocessor_fn(256)
    image = preprocessor(image=image)["image"]
    # image = (image/127.5 - 1.0).astype(np.float32)
    # save the image with the same which is the last part of the path
    # image_name = image_path.split("/")[-1]
    cv2.imwrite(f"/root/data/cropped_sun/depth_{count}.png",image)
    return image
...
def main(gt_dir,pred_dir,gt_list,pred_list):
    ...
    for (gt_path,pred_path) in tqdm(zip(gt_list,pred_list)):
        ...
        gt = preprocess_image(gt_dir+'/'+gt_path, count)
        count += 1
...
if __name__=="__main__":
    gt_dir = "/root/data/SUNRGBD/data_dump/depth" 
    gt_list = os.listdir(gt_dir)

    gt_list = gt_list[1:]

    pred_dir = "/root/data/logs/2023-06-02T21-32-24_dino-ldm-vq-4-semantic/checkpoints/samples/01121472/2023-06-17-02-02-17/img/sample1"
